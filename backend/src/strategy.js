// Proven daily strategy — long-only Connors mean reversion. Mirrors the daily
// branch of assets/js/app/signalEngine.js. Keep the two in sync.
const { sma, rsi, atr, stdev } = require('./indicators');

// candles: [{t,o,h,l,c}] oldest->newest. live: current price (forming day).
function computeSignal(candles, live) {
  ...
  const bWidth = (upperBB - lowerBB) || 1;
  const pctB = (price - lowerBB) / bWidth;
  const prevLow = c[n - 2].l;
  const up = price > s200;

  let setup = 0, conviction = 'normal';
  if (up && rsi2 < 10 && price < prevLow) {
  ...
  return {
    ...
    price,
    plan: verdict === 'BUY'
      ? { entry: price, stop: price - risk, target1: price + risk, risk, riskReward: 1, exitRule: 'firstUpClose', maxHoldMin: 5 * 24 * 60, conviction }
      : null,
    // Latest daily bar's direction — drives the "first up close" exit.
