...
// Intraday bars for the EXPERIMENTAL day-trading engine. The free Yahoo feed only
// serves 15-minute bars going back ~60 days (sub-minute is not available for free —
// that is why scalping stays unavailable). Returns the same { candles, live,
// liveTime } shape as the daily fetch, so the engine and backtester are identical.
export async function fetchIntradayCandles(market, env, { interval = '15m', range = '60d' } = {}) {
  const symbol = market.yahoo;
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=${interval}&range=${range}`;
  const res = await fetch(url, { headers: { 'User-Agent': 'ajent-signals-worker/1.0' } });
  if (!res.ok) throw new Error(`yahoo intraday HTTP ${res.status} ${symbol}`);
  const r = (await res.json())?.chart?.result?.[0];
  if (!r) throw new Error(`yahoo intraday: no data ${symbol}`);
  const q = r.indicators.quote[0], ts = r.timestamp || [], out = [];
  for (let i = 0; i < ts.length; i++) {
    if (q.close[i] == null || q.high[i] == null || q.low[i] == null) continue;
    out.push({ t: ts[i] * 1000, o: q.open[i] ?? q.close[i], h: q.high[i], l: q.low[i], c: q.close[i] });
