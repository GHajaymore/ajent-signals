...
export function processPosition({ symbol, meta, sig, live, open, record, now, risk, cost = 0, exitRsi, dials = null, strat = 'mr', shouldExit }) {
  ...
  if (pos) {
    ...
    record.closed.unshift({ symbol, name: meta.name, side: short ? 'SHORT' : 'LONG', strat: pos.strat || 'mr', entry: pos.entry, exit: price, resultR: +resultR.toFixed(3), pnl, cost, riskDollars: pos.riskDollars || risk, outcome, exitReason: exit, openedAt: pos.openedAt, closedAt: now, adxEntry: pos.adxEntry ?? null });
  ...
  if ((sig.verdict === 'BUY' || sig.verdict === 'SELL') && sig.plan && open) {
    ...
    record.open[symbol] = { symbol, name: meta.name, side: short ? 'SHORT' : 'LONG', strat, entry, stop: short ? entry + r : entry - r, target1: short ? entry - r : entry + r, risk: r, riskDollars, conviction: sig.conviction, maxHoldMin: sig.plan.maxHoldMin, exitRule: strat === 'trend' ? 'trailStop' : 'rsiRecover', exitAbove: sig.plan.exitAbove, peak: entry, openedAt: now, adxEntry: sig.plan.adxEntry ?? null };
    return 'open';
  }
  return 'none';
}
